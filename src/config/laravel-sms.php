...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 如需缓存配置，则需使用 `Surpaimb\Sms\SmsManger::closure($closure)` 方法进行配置
    |
    */
    ...
    |       }
    |
    | 如需缓存配置，则需使用 `Surpaimb\Sms\SmsManger::closure($closure)` 方法对匿名函数进行配置
    |
    */
    'templates' => [
    ],
    ...
    */
    ...
    |
    | driver:
    | 存储方式,是一个实现了'Surpaimb\Sms\Storage'接口的类的类名,
    | 内置可选的值有'Surpaimb\Sms\SessionStorage'和'Surpaimb\Sms\CacheStorage',
    | 如果不填写driver,那么系统会自动根据内置路由的属性(route)中middleware的配置值选择存储器driver:
    | - 如果中间件含有'web',会选择使用'Surpaimb\Sms\SessionStorage'
    | - 如果中间件含有'api',会选择使用'Surpaimb\Sms\CacheStorage'
    |
    | prefix:
    | 存储key的prefix
    |
    | 内置driver的个性化配置:
    | - 在laravel项目的'config/session.php'文件中可以对'Surpaimb\Sms\SessionStorage'进行更多个性化设置
    | - 在laravel项目的'config/cache.php'文件中可以对'Surpaimb\Sms\CacheStorage'进行更多个性化设置
    |
    */
    'storage' => [
        'driver' => '',
    ...
    */
    ...
    |
    */
    'queueJob' => 'Surpaimb\Sms\SendReminderSms',
